...
@class HDGCDAsyncReadPacket;
@class HDGCDAsyncWritePacket;
@class HDGCDAsyncSocketPreBuffer;
...
extern NSString *const HDGCDAsyncSocketException;
extern NSString *const HDGCDAsyncSocketErrorDomain;

extern NSString *const HDGCDAsyncSocketQueueName;
extern NSString *const HDGCDAsyncSocketThreadName;

#if SECURE_TRANSPORT_MAYBE_AVAILABLE
extern NSString *const HDGCDAsyncSocketSSLCipherSuites;
#if TARGET_OS_IPHONE
extern NSString *const HDGCDAsyncSocketSSLProtocolVersionMin;
extern NSString *const HDGCDAsyncSocketSSLProtocolVersionMax;
#else
extern NSString *const HDGCDAsyncSocketSSLDiffieHellmanParameters;
#endif
#endif

enum HDGCDAsyncSocketError
{
	HDGCDAsyncSocketNoError = 0,           // Never used
	HDGCDAsyncSocketBadConfigError,        // Invalid configuration
	HDGCDAsyncSocketBadParamError,         // Invalid parameter was passed
	HDGCDAsyncSocketConnectTimeoutError,   // A connect operation timed out
	HDGCDAsyncSocketReadTimeoutError,      // A read operation timed out
	HDGCDAsyncSocketWriteTimeoutError,     // A write operation timed out
	HDGCDAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	HDGCDAsyncSocketClosedError,           // The remote peer closed the connection
	HDGCDAsyncSocketOtherError,            // Description provided in userInfo
};
typedef enum HDGCDAsyncSocketError HDGCDAsyncSocketError;

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

@interface HDGCDAsyncSocket : NSObject

/**
 * HDGCDAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, HDGCDAsyncSocket will automatically create it's own socket queue.
 ...
 * 
 * For accepting incoming connections, this means HDGCDAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means HDGCDAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, HDGCDAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, HDGCDAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of HDGCDAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a HDGCDAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a HDGCDAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that HDGCDAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - HDGCDAsyncSocketSSLCipherSuites
 * - HDGCDAsyncSocketSSLProtocolVersionMin
 * - HDGCDAsyncSocketSSLProtocolVersionMax
 * 
 * If SecureTransport is available on Mac OS X:
 * 
 * - HDGCDAsyncSocketSSLCipherSuites
 * - HDGCDAsyncSocketSSLDiffieHellmanParameters;
 * 
 * 
...
/**
 * HDGCDAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(HDGCDAsyncSocket *)sock
 * {
 ...
 * Example usage:
 * 
 * - (void)socket:(HDGCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
 * {
...
@protocol HDGCDAsyncSocketDelegate
@optional
...
**/
- (void)socket:(HDGCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(HDGCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(HDGCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(HDGCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(HDGCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(HDGCDAsyncSocket *)sock;
...
**/
- (void)socketDidSecure:(HDGCDAsyncSocket *)sock;
